...
class Trackable {
  ...
  toData(indices) {
    if (indices) return indices.map((index) => this.landmarks[index].toArray());
    return this.landmarks.map((pt) => [pt.x, pt.y]);
  }
